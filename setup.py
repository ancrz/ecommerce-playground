...
def check_python_version() -> bool:
    ...
    if current < MIN_PYTHON_VERSION:
        logger.error(
            f"❌ Python {MIN_PYTHON_VERSION[0]}.{MIN_PYTHON_VERSION[1]}+ requerido. "
...
def run_command(command: list, cwd: Path = None, shell: bool = False) -> bool:
...
def init_database(python_exe: str) -> None:
    ...
    # Create seed script that initializes the database
