        ...
        <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end order-2 md:order-0">
            ...
            {businessInfo?.logo_url ? (
              ...
              !businessInfo?.icon_url && (
                <h1 className="text-xl font-bold">
                  {businessInfo?.name || "E-Commerce"}
                </h1>
              )
            )}
          </Link>

          {/* Mobile Cart/Login (Touch-friendly: min 44x44px) */}
          <div className="flex items-center gap-2 md:hidden">
            ...
            {user ? (
              <Link
                to={accountLink}
                className="p-2 bg-white/20 rounded-lg min-w-[44px] min-h-[44px] flex items-center justify-center"
              >
                <User size={20} />
              </Link>
            ) : (
