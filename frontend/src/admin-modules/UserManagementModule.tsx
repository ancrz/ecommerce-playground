...
export default function UserManagementModule() {
  ...
  const handleSaveUser = async (data: Partial<UserCreateRequest | UserUpdateRequest>, isNew: boolean) => {
    try {
      if (isNew) {
  ...
  return (
    <>
      ...
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Gestión de Usuarios y Roles (RBAC)</h2>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="p-3 text-left text-xs font-semibold uppercase text-gray-600">Usuario</th>
              <th className="p-3 text-left text-xs font-semibold uppercase text-gray-600">Nombre Completo</th>
              <th className="p-3 text-left text-xs font-semibold uppercase text-gray-600">Email</th>
              <th className="p-3 text-left text-xs font-semibold uppercase text-gray-600">Roles</th>
              <th className="p-3 text-center text-xs font-semibold uppercase text-gray-600">Estado</th>
              <th className="p-3 text-center text-xs font-semibold uppercase text-gray-600">Acciones</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              <tr><td colSpan={6} className="text-center p-8 text-gray-500">Cargando usuarios...</td></tr>
            ) : (
              users.map(user => (
                <tr key={user.id} className="hover:bg-gray-50" data-testid={`user-row-${user.id}`}>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {user.roles.map(role => (
                        <span key={role} className={`px-2 py-0.5 text-xs rounded-full font-medium ${
                          role === 'admin' ? 'bg-red-100 text-red-800' : 'bg-gray-200 text-gray-700'
                        }`}>
                          {role}
                        </span>
                      ))}
                  ...
                  <td className="p-3 text-center">
                    {user.is_active ? (
                      <span className="px-2 py-0.5 bg-green-100 text-green-800 text-xs rounded-full font-medium">Activo</span>
                    ) : (
                      <span className="px-2 py-0.5 bg-red-100 text-red-800 text-xs rounded-full font-medium">Inactivo</span>
                    )}
                  </td>
...
const UserFormModal = ({ user, onSave, onCancel }: {
  user: User | null;
  onSave: (data: Partial<UserCreateRequest | UserUpdateRequest>, isNew: boolean) => void;
  onCancel: () => void;
}) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2 pt-2">
        <label className="block text-sm font-semibold text-gray-700">Roles de Acceso</label>
