 ...
 * "Chunk" para la pestaña de Ventas.
 *
 * REFACTORIZADO (FASE 4 - Tracelog v3):
 * - Tabs internas: Punto de Venta | Pedidos
 * - POS: Resumen diario + POS modal + Cola pedidos web + Historial del día
 * - Pedidos: Búsqueda por cédula + rango de fechas + tabla paginada
 */
import {
  useState,
  useEffect,
...
export default function SalesModule() {
  ...
  const loadPendingCarts = useCallback(async () => {
    try {
      const data = await api.getPendingCarts(
        cartPage * CARTS_PER_PAGE,
        CARTS_PER_PAGE + 1,
      );
  ...
  const handleCloseDay = async () => {
    if (
      await confirm(
        "¿Estás seguro de cerrar el día? Esta acción genera el reporte final y no se puede revertir.",
      )
    ) {
      try {
        const report = await api.closeDay();
        await alert(
          `✓ Día cerrado con ${report.sales_count} ventas y un total de ${formatPrice(report.total)}`,
        );
        refreshAll();
      } catch (error: unknown) {
        showToast(
          "Error cerrando el día: " + (error as Error).message,
          "error",
        );
      }
    }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-200 pb-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Punto de venta, pedidos web e historial
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-4">
                <h3 className="font-bold text-gray-900 flex items-center gap-2">
                  <Calendar size={18} className="text-gray-400" /> Acciones de
                  Caja
                </h3>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              {carts.length === 0 ? (
                <div className="p-8 text-center text-gray-400">
                  ...
                  <p className="text-sm">No hay pedidos web pendientes</p>
                </div>
              ) : (
...
const TabButton = ({
  ...
  >
    ...
    {count !== undefined && count > 0 && (
      <span
        className={`px-1.5 py-0.5 rounded-full text-[10px] ${active ? "bg-blue-100 text-blue-700" : "bg-gray-200 text-gray-600"}`}
      >
        {count}
      </span>
    )}
...
// --- Componente: Pestaña de Historial de Pedidos ---
function OrdersHistoryTab() {
  ...
  // Resultados
  const [historyData, setHistoryData] = useState<SalesHistoryResponse | null>(
    null,
  );
  ...
  const fetchHistory = useCallback(
    async (
      ...
      try {
        ...
        console.error(err);
        showToast("Error consultando historial", "error");
      }
      setLoading(false);
    },
    [showToast],
  ...
  // Debounce en búsqueda por cédula (400ms)
  const handleCedulaChange = (value: string) => {
    setSearchCedula(value);
    if (debounceRef.current) clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => {
      setPage(0);
      fetchHistory(0, value, dateFrom, dateTo);
    }, 400);
  };
  ...
  const handleDateChange = (from: string, to: string) => {
    ...
    setDateTo(to);
    setPage(0);
    fetchHistory(0, searchCedula, from, to);
  };

  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    fetchHistory(newPage, searchCedula, dateFrom, dateTo);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm"
            />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wide">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {historyData?.items?.map((sale: Sale) => (
                <tr
                  key={sale.id}
                  className="hover:bg-gray-50/50 transition-colors"
                >
                    ...
                    >
                      ...
                      {searchCedula && (
                        <p className="text-xs mt-1">
                          Intenta con otra cédula o ajusta el rango de fechas
                        </p>
                      )}
                    </td>
                  </tr>
          ...
          <div className="p-4 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg border bg-white text-sm disabled:opacity-50 hover:bg-gray-50 transition-colors"
              >
...
const WebOrderRow = ({
  ...
  const handleComplete = async () => {
    if (
      await confirm(
        `¿Cobrar ${formatPrice(cart.total_with_tax)} a ${cart.customer_name}?`,
        "Confirmar Venta Web",
      )
    ) {
      try {
        await api.completeSale(cart.id, {
  ...
  const handleCancel = async () => {
    if (
      await confirm(`¿Anular pedido de ${cart.customer_name}?`, "Anular Pedido")
    ) {
      try {
      ...
      <div className="flex items-center gap-4 w-full sm:w-auto">
        ...
        <div>
          ...
          <div className="text-xs text-gray-500 font-mono">
            ID: {cart.id.slice(0, 8)} • {cart.item_count} items
          </div>
        </div>
      </div>
...
function POSModal({
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        await alert("Error cargando datos: " + (e as Error).message);
      }
      setLoading(false);
    };
    loadData();
  ...
  const handleCreateCart = async () => {
    ...
    try {
      ...
      await alert("Error creando carrito: " + (e as Error).message);
    }
    setLoading(false);
  };
  ...
  const handleAddItem = async (productId: string, quantity: number) => {
    ...
    try {
      ...
      await alert("Error añadiendo item: " + (e as Error).message);
    }
    setLoading(false);
  };
  ...
  const handleCompleteSale = async () => {
    ...
    try {
      ...
      await alert("Error completando venta: " + (e as Error).message);
    }
    setLoading(false);
  };
  ...
  const handleRecoverCart = async () => {
    ...
    try {
      ...
      await alert(
        "Error recuperando carrito: " + ((e as Error).message || "ID Inválido"),
      );
    } finally {
      setLoading(false);
    }
      ...
      {!cart ? (
        <div className="space-y-6 animate-fade-in" data-testid="pos-setup-view">
          ...
          <div className="relative border-t border-gray-200 pt-6">
            <span className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-2 text-xs text-gray-400 font-bold uppercase tracking-widest">
              O recuperar
            </span>
            <div className="flex gap-2">
              <Input
        ...
        <div
          data-testid="pos-active-cart-view"
            ...
            <div className="p-4 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10">
              ...
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-medium text-red-500">
                  +{formatPrice(cart.tax_amount - cart.igtf_amount)}
                </span>
              </div>
              {cart.igtf_amount > 0 && (
                <div className="flex justify-between text-sm mb-3 animate-fade-in">
                  <span className="text-amber-600 font-medium">
                    IGTF (Impuesto Moneda)
                  </span>
                  <span className="font-medium text-amber-600">
                    +{formatPrice(cart.igtf_amount)}
                  </span>
                </div>
              )}
...
function ProductSearch({
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      await alert("Error buscando: " + (_e as Error).message);
    }
    setLoading(false);
  };
        ...
        <div className="absolute top-full left-0 right-0 mt-2 max-h-60 overflow-y-auto bg-white rounded-xl shadow-xl border border-gray-100 z-50 divide-y divide-gray-50">
          {results.map((prod) => (
            <div
              ...
              onClick={() => prod.stock > 0 && onProductSelect(prod.id, 1)}
            >
              <div>
                ...
                <div className="text-xs text-gray-500">
                  Stock: {prod.stock} • Bs. {prod.price.toFixed(2)}
                </div>
              </div>
