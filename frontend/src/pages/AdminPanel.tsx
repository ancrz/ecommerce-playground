...
const UserManagementModule = lazy(
  () => import("../admin-modules/UserManagementModule"),
);

// Dashboard Lazy Load
const DashboardModule = lazy(() => import("../admin-modules/DashboardModule"));
const SMTPConfigModule = lazy(
  () => import("../admin-modules/SMTPConfigModule"),
);
...
const ModuleIcon = ({
  ...
  if (url) {
    ...
    return (
      <img src={src} alt={`${label} icon`} className="w-5 h-5 object-contain" />
    );
  }
  ...
  const allTabs = useMemo(
    () => [
      // REFACTOR (Punto 4): 'component' es ahora una REFERENCIA, no JSX.
      {
        ...
        iconUrl: customization?.icon_dashboard_url,
        roles: ["admin", "sales_manager", "products_manager"],
      ...
      },
      {
        ...
        label: "Ventas",
        icon: ShoppingCart,
        iconUrl: customization?.icon_pos_url,
      ...
      },
      {
        ...
        label: "Productos",
        icon: Package,
        iconUrl: customization?.icon_products_url,
        roles: ["admin", "products_manager"],
      ...
      },
      {
        ...
        label: "Finanzas",
        icon: DollarSign,
        iconUrl: customization?.icon_finance_url,
      ...
      },
      {
        ...
        label: "Impuestos",
        icon: Percent,
        iconUrl: customization?.icon_tax_url,
        roles: ["admin", "finance_manager"],
        component: TaxModule,
      },
      {
      ...
      }, // Icono temporal
      {
        ...
        label: "SMTP Config",
        icon: Settings,
        iconUrl: customization?.icon_business_url,
        roles: ["admin"],
        component: SMTPConfigModule,
      },
      {
        id: "users",
        label: "Usuarios (RBAC)",
        icon: Users,
        iconUrl: customization?.icon_users_url,
        roles: ["admin"],
      ...
      },
      {
        ...
        icon: Palette,
        iconUrl: customization?.icon_customization_url,
    ...
    [customization],
  ); // 'forceAppUpdate' (función estable) no es dependencia

  // Filtrar pestañas basado en los roles del usuario
  const visibleTabs = useMemo(
    () =>
      allTabs.filter((tab) => tab.roles.some((role) => userRoles.has(role))),
    [allTabs, userRoles],
  );
    ...
    return (
      <div className="text-center p-12" data-testid="admin-no-access">
        ...
        <p className="text-gray-700 mt-2">
          Este usuario no tiene roles de administración asignados.
        </p>
      </div>
    );
  }
