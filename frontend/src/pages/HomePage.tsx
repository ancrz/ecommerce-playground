...
import { Star, Tag, Search, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
...
function ProductSlider({ title, products, customization }: { title: string, products: (Product | ProductCard)[], customization: any }) {
        ...
        {products.length > itemsPerPage && (
          <div className="flex gap-2">
            <button
              ...
              style={{ backgroundColor: customization?.primary_color || '#264192' }}
        ...
        {visibleProducts.map(product => (
          ...
          >
            <div className="relative h-48 bg-gray-100 flex items-center justify-center">
              {product.image_url ? (
                <img 
                  ...
                  src={`${SERVER_URL}${product.image_url}?t=${(product as Product).updated_at || '1'}`} 
                  alt={product.name} 
            ...
            <div className="p-4 flex flex-col flex-grow">
              <h3 className="font-bold text-gray-800 mb-2 h-12 line-clamp-2" title={product.name}>{product.name}</h3>
              ...
              <div className="mb-3 h-8 mt-auto">
                ...
                {product.is_discount ? (
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-xl font-bold text-red-600">{formatPrice(product.final_price)}</span>
                  </div>
                ) : (
                  <span className="text-xl font-bold" style={{ color: customization?.primary_color || '#264192' }}>{formatPrice(product.price)}</span>
                )}
              </div>
...
export default function HomePage() {
  ...
  // REFACTOR: El estado ahora vive DENTRO de la página
  const [mainProducts, setMainProducts] = useState<ProductCard[]>([]);
  const [featuredProducts, setFeaturedProducts] = useState<ProductCard[]>([]);
  const [discountProducts, setDiscountProducts] = useState<ProductCard[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [searchResults, setSearchResults] = useState<(Product | ProductCard)[]>([]);
  const [loadingSearch, setLoadingSearch] = useState(false);
  const [isSearchActive, setIsSearchActive] = useState(false);
  
  // REFACTOR: Cargar datos de sliders al montar
  useEffect(() => {
    const loadSliders = async () => {
      try {
        const [main, featured, discount] = await Promise.all([
          api.getSliderProducts('main'),
          api.getSliderProducts('featured'),
          api.getSliderProducts('discount'),
        ]);
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex gap-2">
            ...
            <button
              ...
              style={{ backgroundColor: customization?.secondary_color || '#ffdd00', color: '#000' }}
            ...
            >
              {loadingSearch ? <Loader2 className="animate-spin" /> : <Search size={20} />}
              Buscar
            </button>
          </div>
        </div>
