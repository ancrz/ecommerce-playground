...
class FinanceService:
    ...
    async def get_currency(self, currency_id: str) -> Currency | None:
        ...
        row = await self.db_manager.fetchone(
            "finance", "SELECT * FROM currencies WHERE id = ? AND is_active = 1", (currency_id,)
        )
    ...
    async def get_base_currency(self) -> Currency | None:
        """Obtener moneda base activa"""
        row = await self.db_manager.fetchone(
            "finance", "SELECT * FROM currencies WHERE is_base = 1 AND is_active = 1 LIMIT 1"
        )
        return self._row_to_currency(row) if row else None

    async def update_exchange_rate(self, currency_id: str, new_rate: Decimal) -> Currency | None:
    ...
    async def delete_currency(self, currency_id: str) -> dict[str, any]:
        ...
        if currency.is_base:
            ...
            raise ValueError("No se puede eliminar la moneda base. Debe asignar una nueva moneda base primero.")
        ...
        return {"success": True, "message": f"Moneda '{currency.name}' desactivada exitosamente."}
    ...
    async def convert_price_to_currency(self, amount_in_base: Decimal, to_currency_id: str) -> dict[str, Any]:
        """
        ...
        if not target_currency or not base_currency:
            logger.error("Error de conversión: Moneda base o destino no encontrada.")
    ...
    def _row_to_currency(self, row: dict) -> Currency | None:
