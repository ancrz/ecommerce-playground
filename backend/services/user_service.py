...
from ..models.base import User, PasswordResetToken, UserCreateRequest, UserUpdateRequest
...
class UserService:
    ...
    async def _row_to_user(self, row: Optional[Dict[str, Any]]) -> Optional[User]:
    ...
    async def create_user(self, user_data: UserCreateRequest) -> User:
        ...
        await self.db_manager.execute(
            ...
            """,
            (
                user.id, user.username, user.password_hash, user.full_name, user.email,
    ...
    async def authenticate_user(self, username: str, plain_password: str) -> Optional[User]:
        """
    ...
    async def update_user_profile(self, user_id: str, updates: UserUpdateRequest) -> User:
        ...
        safe_updates = {
            ...
            "email": updates.email.lower() if updates.email else None
        }
    ...
    async def _create_reset_token(self, user_id: str) -> str:
        ...
        await self.db_manager.execute(
            ...
            """,
            (
                token_model.id, token_model.user_id, token_model.token_hash,
                token_model.expires_at.isoformat(), token_model.is_used,
                token_model.created_at.isoformat(), token_model.updated_at.isoformat()
            )
        )
    ...
    async def _validate_reset_token(self, user_id: str, plain_token: str) -> PasswordResetToken:
        ...
        rows = await self.db_manager.fetchall(
            "password_tokens",
            "SELECT * FROM password_reset_tokens WHERE user_id = ? AND is_used = 0",
            (user_id,)
        )
