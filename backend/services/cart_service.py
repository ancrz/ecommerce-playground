...
from ..models.base import Cart, CartItem, Product
...
class CartService:
    ...
    async def create_cart(
        ...
        try:
            ...
            await self.db_manager.execute("cart", """
                INSERT INTO carts (
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                cart.id, cart.customer_name, cart.customer_id, cart.status,
                cart.region_id, cart.currency_id,
                0, 0, 0, # Totales iniciales
    ...
    async def get_cart(self, cart_id: str) -> Optional[Cart]:
        ...
        items_rows = await self.db_manager.fetchall(
            "cart", "SELECT * FROM cart_items WHERE cart_id = ? ORDER BY product_name",
            (cart_id,)
        )
    ...
    async def add_item(self, cart_id: str, product_id: str, quantity: int) -> Cart:
        ...
        existing_item = await self.db_manager.fetchone(
            "cart", "SELECT * FROM cart_items WHERE cart_id = ? AND product_id = ?",
            (cart_id, product_id)
        )
        ...
        if existing_item:
            ...
            if product.stock < new_quantity:
                raise ValueError(f"Stock insuficiente para {product.name} (Solicitado: {new_quantity}, Stock: {product.stock})")
            
            await self.db_manager.execute(
                "cart", "UPDATE cart_items SET quantity = ?, price = ? WHERE cart_id = ? AND product_id = ?",
                (new_quantity, float(item_price), cart_id, product_id)
            )
        else:
            await self.db_manager.execute(
                "cart", "INSERT INTO cart_items (cart_id, product_id, product_name, quantity, price) VALUES (?, ?, ?, ?, ?)",
    ...
    async def remove_item(self, cart_id: str, product_id: str) -> Cart:
        ...
        await self.db_manager.execute(
            "cart", "DELETE FROM cart_items WHERE cart_id = ? AND product_id = ?",
            (cart_id, product_id)
        )
        return await self._update_cart_totals(cart_id)
    ...
    async def update_item_quantity(self, cart_id: str, product_id: str, new_quantity: int) -> Cart:
        ...
        await self.db_manager.execute(
            "cart", "UPDATE cart_items SET quantity = ? WHERE cart_id = ? AND product_id = ?",
    ...
    async def _update_cart_totals(self, cart_id: str) -> Cart:
        ...
        subtotal_row = await self.db_manager.fetchone(
            "cart", "SELECT SUM(price * quantity) as subtotal FROM cart_items WHERE cart_id = ?",
            (cart_id,)
        )
        ...
        logger.info(f"Carrito {cart_id} recalculado: Subtotal={subtotal}, Impuestos={tax_amount}, Total={total_with_tax}")
    ...
    async def generate_qr(self, cart_id: str) -> str:
        ...
        # 3. Guardar en BD
        await self.db_manager.execute(
            "cart", "UPDATE carts SET qr_code = ? WHERE id = ?",
    ...
    async def get_pending_carts(self) -> List[Cart]:
        ...
        rows = await self.db_manager.fetchall("cart", "SELECT * FROM carts WHERE status = 'pending' ORDER BY created_at DESC")
