...
from ..models.base import Sale, Cart, DailyReport, PaymentDetails
...
class SalesService:
    ...
    async def complete_sale(
        ...
        await self.db_manager.execute("sales", """
            INSERT INTO sales (
            ...
            """,
            (
                sale.id, sale.cart_id, sale.customer_name, sale.customer_id,
                ...
                sale.status, sale.completed_by, sale.completed_at.isoformat(),
                sale.region_id, float(sale.subtotal), float(sale.tax_amount), 
    ...
    async def get_daily_report(self) -> DailyReport:
        ...
        rows = await self.db_manager.fetchall("sales", """
            SELECT * FROM sales 
    ...
    async def close_day_report(self, closed_by: str) -> Dict[str, Any]:
        ...
        existing = await self.db_manager.fetchone("sales", """
            SELECT * FROM daily_closures WHERE date = ?""", (today,)
        )
        if existing:
