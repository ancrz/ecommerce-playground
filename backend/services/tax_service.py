...
from ..models.base import Region, TaxRate
...
class TaxService:
    ...
    async def create_region(self, name: str, country: str, state: str, city: Optional[str] = None, zip_code: Optional[str] = None) -> Region:
        ...
        try:
            await self.db_manager.execute("tax", """
                INSERT INTO regions (id, name, country, state, city, zip_code, is_active, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                region.id, region.name, region.country, region.state, region.city,
                region.zip_code, region.is_active,
                region.created_at.isoformat(), region.updated_at.isoformat()
            ))
    ...
    async def update_region(self, region_id: str, updates: Dict[str, Any]) -> Optional[Region]:
    ...
    async def create_tax_rate(self, name: str, region_id: str, rate: Decimal, priority: int = 1) -> TaxRate:
        ...
        try:
            await self.db_manager.execute("tax", """
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                tax_rate.id, tax_rate.name, tax_rate.region_id, float(tax_rate.rate),
                tax_rate.priority, tax_rate.is_active,
                tax_rate.created_at.isoformat(), tax_rate.updated_at.isoformat()
            ))
    ...
    async def get_tax_rates_for_region(self, region_id: str) -> List[TaxRate]:
        ...
        rows = await self.db_manager.fetchall(
            "tax",
            "SELECT * FROM tax_rates WHERE region_id = ? AND is_active = 1 ORDER BY priority",
            (region_id,)
        )
        return [self._row_to_tax_rate(row) for row in rows]
        
    async def get_tax_rate(self, tax_rate_id: str) -> Optional[TaxRate]:
    ...
    async def update_tax_rate(self, tax_rate_id: str, updates: Dict[str, Any]) -> Optional[TaxRate]:
    ...
    async def calculate_taxes(self, subtotal: Decimal, region_id: str) -> Dict[str, Any]:
        ...
        for rate in rates:
            ...
            breakdown.append({
                ...
                "amount": float(tax_amount_for_this_rate)
            })

        total_tax = total_tax.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        total_with_tax = subtotal + total_tax
        
        logger.info(f"Impuesto calculado para Región {region_id}: Subtotal {subtotal}, Impuesto {total_tax}, Total {total_with_tax}")
    ...
    def _row_to_region(self, row: Dict) -> Optional[Region]:
        if not row:
            return None
        try:
    ...
    def _row_to_tax_rate(self, row: Dict) -> Optional[TaxRate]:
        if not row:
            return None
        try:
