...
from ..models.base import (
    LoginRequest, TokenResponse, User, UserPublic, # Add UserPublic here
    PasswordChangeRequest, UserUpdateRequest,
    PasswordResetRequest, PasswordResetValidate
)
...
async def validate_password_reset(
    request: PasswordResetValidate = Body(...),
    service: UserService = Depends(get_user_service)
):
    """
...
async def get_me(
    ...
    if not user_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Token inválido: ID de usuario no encontrado.")
...
async def update_me(
    ...
    if updates.roles is not None or updates.is_active is not None:
        raise HTTPException(
            ...
            detail="No puede cambiar sus propios roles o estado de activación."
        )
