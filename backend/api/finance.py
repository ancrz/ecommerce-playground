...
from ..utils.auth import get_current_user, is_finance_manager
...
async def get_price_conversion(
    ...
    """
    try:
        return await service.convert_price_to_currency(
...
@router.post("/currencies", response_model=Currency, status_code=201,
...
@router.put("/currencies/{currency_id}/rate", response_model=Currency,
            dependencies=[Depends(is_finance_manager)]) # RBAC
async def update_exchange_rate(
    currency_id: str,
    new_rate: float = Query(..., gt=0),
    service: FinanceService = Depends(get_finance_service)
):
    """
...
@router.put("/currencies/{currency_id}/set-base", response_model=Dict[str, Any],
            dependencies=[Depends(is_finance_manager)]) # RBAC
...
@router.delete("/currencies/{currency_id}", response_model=Dict[str, Any],
              dependencies=[Depends(is_finance_manager)]) # RBAC
