...
from fastapi import (  # <-- CORRECCIÓN FASE 1: Importar Request
    APIRouter,
    Depends,
    File,
    Form,
...
# Importar DTOs
from ..models import BusinessInfo, Product, ProductUpdate
...
async def upload_product_image(
    ...
    try:
        ...
        updated_product = await product_service.update_product(
            product_id, updates=update_dto.model_dump(exclude_none=True)
        )
...
async def delete_product_image(
    ...
    try:
        ...
        updated_product = await product_service.update_product(product_id, updates={"image_url": None})
        return updated_product
...
async def get_product_gallery(product_id: str, product_service: ProductService = Depends(get_product_service)):
    """
...
async def upload_to_product_gallery(
    ...
    try:
        ...
        image_url = image_service.process_and_save_multi_size(
            file_data=file_data, original_filename=file.filename, save_filename=filename, folder="products"
        )
        ...
        result = await product_service.add_product_image(
            product_id=product_id, image_url=image_url, thumbnail_url=thumbnail_url, is_main=is_main, alt_text=alt_text
        )
